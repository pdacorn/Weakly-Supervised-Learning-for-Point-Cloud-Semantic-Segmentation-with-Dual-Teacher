...
def crop_pc(coord, feat, label, mask, split='train',
    ...
    if voxel_max is not None:
        ...
        coord, feat, label, mask = coord[crop_idx], feat[crop_idx] if feat is not None else None, label[crop_idx] if label is not None else None, mask[crop_idx] if mask is not None else None
    coord -= coord.min(0)
    return coord.astype(np.float32), feat.astype(np.float32) if feat is not None else None, \
           label.astype(np.long) if label is not None else None, mask.astype(np.long) if mask is not None else None
